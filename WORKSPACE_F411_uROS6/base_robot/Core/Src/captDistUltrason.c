...
/*
		...
		ATTENDRE 100ms avant la prochaine mesure
 */
//=================================================================
//			SRF02 GET DISTANCE
//=================================================================
...
void Ultrason(int *dist)
{
	//Measure
	uint8_t 	data_to_write;
	data_to_write=0x51;
	i2c1_WriteRegBuffer(CAPT_US_LEFT_ADDRESS, 0,  &data_to_write, 1);
	i2c1_WriteRegBuffer(CAPT_US_RIGHT_ADDRESS, 0,  &data_to_write, 1);
	HAL_Delay(70);
	...
	i2c1_ReadRegBuffer(CAPT_US_LEFT_ADDRESS, 0x02, &dist_h, 1);
	i2c1_ReadRegBuffer(CAPT_US_LEFT_ADDRESS, 0x03, &dist_l, 1);

	dist[0]=(uint16_t)(((uint16_t)(dist_h)<< 8)+ (uint16_t)dist_l);

	i2c1_ReadRegBuffer(CAPT_US_RIGHT_ADDRESS, 0x02, &dist_h, 1);
	i2c1_ReadRegBuffer(CAPT_US_RIGHT_ADDRESS, 0x03, &dist_l, 1);

	dist[1]=(uint16_t)(((uint16_t)(dist_h)<< 8)+ (uint16_t)dist_l);
}
